...
function revealMods(mods) {
    ...
    function revealMod_(modVal, modName) {
        return _.isPlainObject(modVal) && revealMods(modVal)
            || _.isString(modVal) && {mod: utils.createMapEntry(modVal, true)}
            || !_.isArray(modVal) && {mod: utils.createMapEntry(modName, modVal)}
            || _.map(modVal, function(val) {
